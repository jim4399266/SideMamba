...
def selective_scan_ref(u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False,
    ...
    out = y if D is None else y + u * rearrange(D, "dilation -> dilation 1")
    if z is not None:
        out = out * F.silu(z)
...
class MambaInnerFnNoOutProj(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        if xz.stride(-1) != 1:
            xz = xz.contiguous()
        conv1d_weight = rearrange(conv1d_weight, "dilation 1 w -> dilation w")
        ...
        # We want delta to have dilation as the slowest moving dimension
        # and L as the fastest moving dimension, since those are what the ssm_scan kernel expects.
        x_dbl = F.linear(rearrange(conv1d_out, 'b dilation l -> (b l) dilation'), x_proj_weight)  # (bl dilation)
        delta = rearrange(delta_proj_weight @ x_dbl[:, :delta_rank].t(), "dilation (b l) -> b dilation l", l = L)
        ...
        # return rearrange(out_z, "b dilation l -> b l dilation")
        return out_z
    ...
    def backward(ctx, dout):
        ...
        if ctx.checkpoint_lvl == 1:
            ...
            delta = rearrange(delta_proj_weight @ x_dbl[:, :delta_rank].t(),
                              "dilation (b l) -> b dilation l", l = L)
        # The kernel supports passing in a pre-allocated dz (e.g., in case we want to fuse the
        # backward of selective_scan_cuda with the backward of chunk).
        dxz = torch.empty_like(xz)  # (batch, dim, seqlen)
        dx, dz = dxz.chunk(2, dim=1)
        # dout_y = rearrange(dout, "b l dilation -> b dilation l") # because no arrange at end of forward, so dout shape is b dilation l
        ...
        if ctx.is_variable_B:
            ...
            dx_dbl[:, delta_rank:delta_rank + d_state] = dB  # (bl dilation)
            dB = None
        dC_proj_bias = None
        if ctx.is_variable_C:
            ...
            dx_dbl[:, -d_state:] = dC  # (bl dilation)
            dC = None
        ddelta = rearrange(ddelta, "b dilation l -> dilation (b l)")
        ddelta_proj_weight = torch.einsum("dB,Br->dr", ddelta, x_dbl[:, :delta_rank])
        dx_dbl[:, :delta_rank] = torch.einsum("dB,dr->Br", ddelta, delta_proj_weight)
        dconv1d_out = rearrange(dconv1d_out, "b dilation l -> dilation (b l)")
        dx_proj_weight = torch.einsum("Br,Bd->rd", dx_dbl, rearrange(conv1d_out, "b dilation l -> (b l) dilation"))
        dconv1d_out = torch.addmm(dconv1d_out, x_proj_weight.t(), dx_dbl.t(), out=dconv1d_out)
        dconv1d_out = rearrange(dconv1d_out, "dilation (b l) -> b dilation l", b=x.shape[0], l=x.shape[-1])
        ...
        dconv1d_weight = rearrange(dconv1d_weight, "dilation w -> dilation 1 w")
...
class MambaInnerFn(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        if dt_rms_weight is not None:
            delta = rearrange(delta, "b dilation l -> (b l) dilation", l=L).contiguous()
            delta = rms_norm_forward(delta, dt_rms_weight, bias=None, eps=b_c_dt_rms_eps)
            delta = rearrange(delta, "(b l) dilation -> b dilation l", l=L).contiguous()
        ...
        return F.linear(rearrange(out_z, "b dilation l -> b l dilation"), out_proj_weight, out_proj_bias)
    ...
    def backward(ctx, dout):
        ...
        dout_y = rearrange(out_proj_weight.t() @ dout, "dilation (b l) -> b dilation l", l=L)
        dconv1d_out, ddelta, dA, dB, dC, dD, ddelta_bias, dz, out_z = selective_scan_cuda.bwd(
            conv1d_out, delta, A, B, C, D, z, delta_bias, dout_y, scan_intermediates, out, dz,
            ctx.delta_softplus,
            True  # option to recompute out_z
        )
        dout_proj_weight = torch.einsum("eB,dB->ed", dout, rearrange(out_z, "b dilation l -> dilation (b l)"))
...
class BiMambaInnerFn(torch.autograd.Function):
    ...
    def forward(ctx, xz, conv1d_weight, conv1d_bias, x_proj_weight, delta_proj_weight,
        ...
        ctx.save_for_backward(xz, conv1d_weight, conv1d_bias, x_dbl, x_proj_weight,
                              delta_proj_weight, out_proj_weight, conv1d_out, delta,
                              A, A_b, B, C, D, delta_bias, scan_intermediates_f, scan_intermediates_b,
                              b_rms_weight, c_rms_weight, dt_rms_weight, out_f, out_b)
        return F.linear(rearrange(out_z, "b dilation l -> b l dilation"), out_proj_weight, out_proj_bias)
    ...
    def backward(ctx, dout):
        # dout: (batch, seqlen, dim)
        (xz, conv1d_weight, conv1d_bias, x_dbl, x_proj_weight, delta_proj_weight, out_proj_weight,
         conv1d_out, delta, A, A_b, B, C, D, delta_bias, scan_intermediates_f, scan_intermediates_b,
         b_rms_weight, c_rms_weight, dt_rms_weight, out_f, out_b) = ctx.saved_tensors
        L = xz.shape[-1]
        delta_rank = delta_proj_weight.shape[1]
        d_state = A.shape[-1] * (1 if not A.is_complex() else 2)
        x, z = xz.chunk(2, dim=1)
...
def mamba_inner_ref(
    ...
    d_state = A.shape[-1] * (1 if not A.is_complex() else 2)
    x, z = xz.chunk(2, dim=1)
    x = causal_conv1d_fn(x, rearrange(conv1d_weight, "dilation 1 w -> dilation w"), conv1d_bias, activation="silu")
    # We're being very careful here about the layout, to avoid extra transposes.
    # We want delta to have dilation as the slowest moving dimension
    # and L as the fastest moving dimension, since those are what the ssm_scan kernel expects.
    x_dbl = F.linear(rearrange(x, 'b dilation l -> (b l) dilation'), x_proj_weight)  # (bl dilation)
    delta = delta_proj_weight @ x_dbl[:, :delta_rank].t()
    delta = rearrange(delta, "dilation (b l) -> b dilation l", l=L)
    if B is None:  # variable B
        B = x_dbl[:, delta_rank:delta_rank + d_state]  # (bl dilation)
        if B_proj_bias is not None:
            B = B + B_proj_bias.to(dtype=B.dtype)
        if not A.is_complex():
            B = rearrange(B, "(b l) dstate -> b dstate l", l=L).contiguous()
    ...
    return F.linear(rearrange(y, "b dilation l -> b l dilation"), out_proj_weight, out_proj_bias)
...
def bimamba_inner_ref(
    ...
    d_state = A.shape[-1] * (1 if not A.is_complex() else 2)
    x, z = xz.chunk(2, dim=1)
    x = causal_conv1d_fn(x, rearrange(conv1d_weight, "dilation 1 w -> dilation w"), conv1d_bias, "silu")
    # We're being very careful here about the layout, to avoid extra transposes.
    # We want delta to have dilation as the slowest moving dimension
    # and L as the fastest moving dimension, since those are what the ssm_scan kernel expects.
    x_dbl = F.linear(rearrange(x, 'b dilation l -> (b l) dilation'), x_proj_weight)  # (bl dilation)
