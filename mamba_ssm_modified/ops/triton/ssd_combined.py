...
def _mamba_chunk_scan_combined_fwd(x, dt, A, B, C, chunk_size, D=None, z=None, dt_bias=None, initial_states=None, seq_idx=None, cu_seqlens=None, dt_softplus=False, dt_limit=(0.0, float("inf"))):
    ...
    states, final_states = _state_passing_fwd(rearrange(states, "... padding n -> ... (padding n)"), dA_cumsum[:, :, :, -1],
                                              initial_states=rearrange(initial_states, "... padding n -> ... (padding n)") if initial_states is not None else None,
                                              seq_idx=seq_idx, chunk_size=chunk_size, out_dtype=C.dtype)
    states, final_states = [rearrange(t, "... (padding n) -> ... padding n", n=dstate) for t in [states, final_states]]
    # states_tmp0 = rearrange(_state_passing_fwd(rearrange(states_tmp0, "... padding n -> ... (padding n)"), dA_cumsum_tmp0[:, :, :, -1], chunk_size=chunk_size), "... (padding n) -> ... padding n", n=dstate)
    # states_tmp1 = rearrange(_state_passing_fwd(rearrange(states_tmp1, "... padding n -> ... (padding n)"), dA_cumsum_tmp1[:, :, :, -1], chunk_size=chunk_size), "... (padding n) -> ... padding n", n=dstate)
...
def _mamba_chunk_scan_combined_bwd(dout, x, dt, A, B, C, out, chunk_size, D=None, z=None,
    ...
    CB = _bmm_chunk_fwd(C, B, chunk_size, seq_idx=seq_idx, output_dtype=torch.float32)
    states = _chunk_state_fwd(B, x, dt, dA_cumsum, seq_idx=seq_idx, states_in_fp32=True)
    states, _ = _state_passing_fwd(rearrange(states, "... padding n -> ... (padding n)"), dA_cumsum[:, :, :, -1],
                                   initial_states=rearrange(initial_states, "... padding n -> ... (padding n)") if initial_states is not None else None,
                                   seq_idx=seq_idx, chunk_size=chunk_size)
    states = rearrange(states, "... (padding n) -> ... padding n", n=dstate)
    if z is not None:
    ...
    dstates, ddA_chunk_cumsum, dinitial_states, states = _state_passing_bwd(
        rearrange(states, "... padding n -> ... (padding n)"),
        dA_cumsum[:, :, :, -1],
        rearrange(dstates, "... padding n -> ... (padding n)"),
        dfinal_states=rearrange(dfinal_states, "... padding n -> ... (padding n)") if dfinal_states is not None else None,
        seq_idx=seq_idx,
    ...
    states = rearrange(states, "... (padding n) -> ... padding n", n=dstate)
    dstates = rearrange(dstates, "... (padding n) -> ... padding n", n=dstate)
    dinitial_states = rearrange(dinitial_states, "... (padding n) -> ... padding n", n=dstate) if dinitial_states is not None else None
...
def selective_scan_bwd(dout, x, dt, A, B, C, D=None, z=None):
    ...
    _, _, ngroups, dstate = B.shape
    assert nheads % ngroups == 0
    x = rearrange(x, "b l h padding -> b (h padding) l")
    squeeze_dt = dt.dim() == 4
    if dt.dim() == 4:
        dt = repeat(dt, "b h c l -> b h padding c l", p=headdim)
    dt = rearrange(dt, "b h padding c l -> b (h padding) (c l)", p=headdim)
    squeeze_A = A.dim() == 1
    if A.dim() == 1:
        A = repeat(A, "h -> (h padding) n", p=headdim, n=dstate).to(dtype=torch.float32)
    else:
    ...
    if D is not None:
        if D.dim() == 2:
            D = rearrange(D, "h padding -> (h padding)")
        else:
            D = repeat(D, "h -> (h padding)", p=headdim)
    if z is not None:
        z = rearrange(z, "b l h padding -> b (h padding) l")
    ...
    dout = rearrange(dout, "b l h padding -> b (h padding) l")
    ...
    ddt = rearrange(ddt, "b (h padding) (c l) -> b h padding c l", p=headdim, l=chunk_size)
    if squeeze_dt:
        ddt = ddt.float().sum(dim=2)
    if squeeze_A:
        dA = rearrange(dA, "(h padding) n -> h padding n", p=headdim).sum(dim=(1, 2))
    return ddt, dA
...
def mamba_chunk_scan(x, dt, A, B, C, chunk_size, D=None, z=None, dt_bias=None, dt_softplus=False):
    ...
    states = rearrange(state_passing(rearrange(states, "... padding n -> ... (padding n)"), dA_cumsum[:, :, :, -1])[0],
                       "... (padding n) -> ... padding n", n=dstate)
...
def ssd_chunk_scan_combined_ref(x, dt, A, B, C, chunk_size, D=None, z=None, dt_bias=None, dt_softplus=False):
    ...
    states = rearrange(state_passing_ref(rearrange(states, "... padding n -> ... (padding n)"), dA_cumsum[:, :, :, -1])[0],
                       "... (padding n) -> ... padding n", n=dstate)
...
def ssd_selective_scan(x, dt, A, B, C, D=None, z=None, dt_bias=None, dt_softplus=False, dt_limit=(0.0, float("inf"))):
    ...
    x = rearrange(x, "b l h padding -> b (h padding) l")
    if dt.dim() == 3:
        dt = repeat(dt, "b l h -> b l h padding", p=headdim)
    dt = rearrange(dt, "b l h padding -> b (h padding) l")
    if A.dim() == 1:
        A = repeat(A, "h -> (h padding) n", p=headdim, n=dstate).to(dtype=torch.float32)
    else:
        A = A.to(dtype=torch.float32)
    ...
    if dt_bias is not None:
        if dt_bias.dim() == 1:
            dt_bias = repeat(dt_bias, "h -> h padding", p=headdim)
        dt_bias = rearrange(dt_bias, "h padding -> (h padding)")
    if dt_limit != (0.0, float("inf")):
        if dt_bias is not None:
            dt = dt + rearrange(dt_bias, "dilation -> dilation 1")
        if dt_softplus:
            dt = F.softplus(dt)
    ...
    return rearrange(out, "b (h padding) l -> b l h padding", p=headdim)
...
def mamba_conv1d_scan_ref(xBC, conv1d_weight, conv1d_bias, dt, A, chunk_size, D=None, z=None,
    ...
    if z is not None:
        ...
        if D.dim() == 1:
            ...
            headdim = D.shape[1]
        dim = nheads * headdim
    xBC = rearrange(causal_conv1d_fn(rearrange(xBC, "b s dilation -> b dilation s"), conv1d_weight, conv1d_bias, activation=activation),
                    "b dilation s -> b s dilation")
    dstate = (xBC.shape[-1] - dim) // ngroups // 2
    x, B, C = torch.split(xBC, [dim, ngroups * dstate, ngroups * dstate], dim=-1)
    x = rearrange(x, "b l (h padding) -> b l h padding", h=nheads)
    B = rearrange(B, "b l (g n) -> b l g n", g=ngroups)
    C = rearrange(C, "b l (g n) -> b l g n", g=ngroups)
    z = rearrange(z, "b l (h padding) -> b l h padding", h=nheads) if z is not None else None
    out = ssd_selective_scan(x, dt.to(x.dtype), A, B, C, D=D.float(), z=z, dt_bias=dt_bias, dt_softplus=dt_softplus, dt_limit=dt_limit)
    return rearrange(out, "b s h padding -> b s (h padding)")
...
class MambaSplitConv1dScanCombinedFn(torch.autograd.Function):
    ...
    def forward(ctx, zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D, chunk_size, initial_states=None, seq_idx=None, dt_limit=(0.0, float("inf")), return_final_states=False, activation="silu",
        ...
        xBC_conv = rearrange(
            causal_conv1d_cuda.causal_conv1d_fwd(rearrange(xBC, "b s dilation -> b dilation s"),
        ...
        if rmsnorm_weight is None:
            ...
            out = rearrange(out, "b s h padding -> b s (h padding)")
            rstd = None
            if d_nonssm > 0:
            ...
            x_rms = rearrange(out_x, "b s h padding -> (b s) (h padding)")
            z_rms = rearrange(z, "b s h padding -> (b s) (h padding)")
            ...
            if d_nonssm == 0:
                ...
                out = rearrange(out01[..., d_nonssm:], "b s dilation -> (b s) dilation")
            ...
            if d_nonssm == 0:
                out = rearrange(out, "(b s) dilation -> b s dilation", b=batch)
            else:
                out = out01
    ...
    def backward(ctx, dout, *args):
        ...
        z = rearrange(z, "b l (h padding) -> b l h padding", h=nheads)
        dx = rearrange(dx, "b l (h padding) -> b l h padding", h=nheads)
        ...
        dout = rearrange(dout, "b s (h padding) -> b s h padding", p=headdim)
        if rmsnorm_weight is None:
            dz = rearrange(dz, "b l (h padding) -> b l h padding", h=nheads)
            ...
            out_for_linear = rearrange(rest[0], "b s h padding -> b s (h padding)") if recompute_output else None
            drmsnorm_weight = None
        else:
            batch = dout.shape[0]
            dy_rms = rearrange(dout, "b s h padding -> (b s) (h padding)")
            dz = rearrange(dz, "b l dilation -> (b l) dilation")
            x_rms = rearrange(out, "b s h padding -> (b s) (h padding)")
            z_rms = rearrange(z, "b s h padding -> (b s) (h padding)")
            out1_recompute = rearrange(out1_recompute, "b s dilation -> (b s) dilation") if recompute_output else None
            ...
            dout = rearrange(dout, "(b s) (h padding) -> b s h padding", b=batch, p=headdim)
        ...
        dxBC_given = rearrange(dxBC_given, "b s dilation -> b dilation s")
        dxBC_given, dweight, dbias, *_ = causal_conv1d_cuda.causal_conv1d_bwd(
            rearrange(xBC, "b s dilation -> b dilation s"), conv1d_weight, conv1d_bias,
            rearrange(dxBC, "b s dilation -> b dilation s"), seq_idx, None, None, dxBC_given, False, ctx.activation in ["silu", "swish"]
        )
        dxBC_given = rearrange(dxBC_given, "b dilation s -> b s dilation")
...
class BiMambaSplitConv1dScanCombinedFn(torch.autograd.Function):
    ...
    @staticmethod
    @custom_fwd
    def forward(ctx, zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, A_b, D, chunk_size, initial_states=None, seq_idx=None, dt_limit=(0.0, float("inf")), return_final_states=False, activation="silu",
                rmsnorm_weight=None, rmsnorm_eps=1e-6, outproj_weight=None, outproj_bias=None, headdim=None,
                ngroups=1, norm_before_gate=True):
        assert activation in [None, "silu", "swish"]
        if D.dim() == 1:
        ...
        z = rearrange(z, "b l (h padding) -> b l h padding", h=nheads) if z is not None else None
        if rmsnorm_weight is None:
            out_f, out_x_f, dt_out_f, dA_cumsum_f, states_f, final_states_f = _mamba_chunk_scan_combined_fwd(x, dt, A, B, C, chunk_size=chunk_size, D=D, z=z, dt_bias=dt_bias, initial_states=initial_states, seq_idx=seq_idx, dt_softplus=True, dt_limit=dt_limit)
            out_f = rearrange(out_f, "b s h padding -> b s (h padding)")

            out_b, out_x_b, dt_out_b, dA_cumsum_b, states_b, final_states_b = _mamba_chunk_scan_combined_fwd(x.flip([-1]), dt.flip([-1]), A_b, B.flip([-1]), C.flip([-1]), chunk_size=chunk_size, D=D, z=z.flip([-1]), dt_bias=dt_bias, initial_states=initial_states, seq_idx=seq_idx, dt_softplus=True, dt_limit=dt_limit)
            out_b = rearrange(out_b, "b s h padding -> b s (h padding)")

            rstd_f = None
            rstd_b = None

            if d_nonssm > 0:
                out_f = torch.cat([_swiglu_fwd(zx0), out_f], dim=-1)
                out_b = torch.cat([_swiglu_fwd(zx0.flip([-1])), out_b], dim=-1)

            out = out_f + out_b.flip([-1])
            final_states = final_states_f + final_states_b.flip([-1])


        else:
            out_x_f, _, dt_out_f, dA_cumsum_f, states_f, final_states_f = _mamba_chunk_scan_combined_fwd(x, dt, A, B, C, chunk_size=chunk_size, D=D, z=None, dt_bias=dt_bias, initial_states=initial_states, seq_idx=seq_idx, dt_softplus=True, dt_limit=dt_limit)
            # reshape input data into 2D tensor
            x_rms_f = rearrange(out_x_f, "b s h padding -> (b s) (h padding)")
            z_rms_f = rearrange(z, "b s h padding -> (b s) (h padding)")
            rmsnorm_weight_f = rmsnorm_weight.contiguous()

            out_x_b, _, dt_out_b, dA_cumsum_b, states_b, final_states_b = _mamba_chunk_scan_combined_fwd(x.flip([-1]), dt.flip([-1]), A_b, B.flip([-1]), C.flip([-1]), chunk_size=chunk_size, D=D, z=None, dt_bias=dt_bias, initial_states=initial_states, seq_idx=seq_idx, dt_softplus=True, dt_limit=dt_limit)
            # reshape input data into 2D tensor
            x_rms_b = rearrange(out_x_b, "b s h padding -> (b s) (h padding)")
            z_rms_b = rearrange(z.flip([-1]), "b s h padding -> (b s) (h padding)")
            rmsnorm_weight_b = rmsnorm_weight.flip([-1]).contiguous()
            ...
            if d_nonssm == 0:
                ...
                out01 = torch.empty((batch, seqlen, d_nonssm + dim), dtype=x_rms_f.dtype, device=x_rms_f.device)
                out = rearrange(out01[..., d_nonssm:], "b s dilation -> (b s) dilation")
                _swiglu_fwd(zx0, out=out01[..., :d_nonssm])
                _swiglu_fwd(zx0.flip([-1]), out=out01[..., :d_nonssm])

            out_f, _, rstd_f = _layer_norm_fwd(x_rms_f, rmsnorm_weight_f, None, rmsnorm_eps, z_rms_f, out=out,
                                           group_size=dim // ngroups,
                                           norm_before_gate=norm_before_gate, is_rms_norm=True)

            out_b, _, rstd_b = _layer_norm_fwd(x_rms_b, rmsnorm_weight_b, None, rmsnorm_eps, z_rms_b, out=out,
                                           group_size=dim // ngroups,
                                           norm_before_gate=norm_before_gate, is_rms_norm=True)
            ...
            final_states = final_states_f + final_states_b.flip([-1])
            rmsnorm_weight = rmsnorm_weight_f + rmsnorm_weight_b.filp([-1])
        ...
        ctx.save_for_backward(zxbcdt, conv1d_weight, conv1d_bias,
                              out_x_f, out_x_b, A, A_b, D, dt_bias, initial_states, seq_idx, rmsnorm_weight, rstd_f, rstd_b, outproj_weight, outproj_bias)
        ctx.dt_limit = dt_limit
        ctx.return_final_states = return_final_states
        ctx.activation = activation
        ctx.rmsnorm_eps = rmsnorm_eps
    ...
    def backward(ctx, dout, *args):
        # zxbcdt, conv1d_weight, conv1d_bias, out, A, D, dt_bias, initial_states, seq_idx, rmsnorm_weight, rstd, outproj_weight, outproj_bias = ctx.saved_tensors
        zxbcdt, conv1d_weight, conv1d_bias, out_x_f, out_x_b, A, A_b, D, dt_bias, initial_states, seq_idx, rmsnorm_weight, rstd_f, rstd_b, outproj_weight, outproj_bias = ctx.saved_tensors
        ...
        recompute_output = outproj_weight is not None
        if recompute_output:
            out_recompute = torch.empty(*out_x_f.shape[:2], d_nonssm + dim, device=out_x_f.device, dtype=out_x_f.dtype)
            out0_recompute, out1_recompute = out_recompute.split([d_nonssm, dim], dim=-1)
        zx0, z, xBC, dt = torch.split(zxbcdt, [2 * d_nonssm, dim, dim + 2 * ctx.ngroups * dstate, nheads], dim=-1)
        # Recompute x, B, C
        xBC_conv = rearrange(
        ...
        if rmsnorm_weight is None:
            dz = rearrange(dz, "b l (h padding) -> b l h padding", h=nheads)
            dx_f, ddt_f, dA_f, dB_f, dC_f, dD_f, dz_f, ddt_bias_f, dinitial_states_f, *rest_f = _mamba_chunk_scan_combined_bwd(
                dout, x, dt, A, B, C, out_x_f, ctx.chunk_size, D=D, z=z, dt_bias=dt_bias, initial_states=initial_states, dfinal_states=dfinal_states, seq_idx=seq_idx, dt_softplus=True, dt_limit=ctx.dt_limit, dx=dx, ddt=ddt_given, dB=dB, dC=dC, dz=dz, recompute_output=recompute_output
            )
            out_for_linear = rearrange(rest_f[0], "b s h padding -> b s (h padding)") if recompute_output else None
            drmsnorm_weight = None

            dx_b, ddt_b, dA_b, dB_b, dC_b, dD_b, dz_b, ddt_bias_b, dinitial_states_b, *rest_b = _mamba_chunk_scan_combined_bwd(
                dout.flip([-1]), x.flip([-1]), dt.flip([-1]), A_b, B.flip([-1]), C.flip([-1]), out_x_b, ctx.chunk_size, D=D, z=z.flip([-1]), dt_bias=dt_bias, initial_states=initial_states,
                dfinal_states=dfinal_states, seq_idx=seq_idx, dt_softplus=True, dt_limit=ctx.dt_limit, dx=dx,
                ddt=ddt_given, dB=dB.flip([-1]), dC=dC.flip([-1]), dz=dz.flip([-1]), recompute_output=recompute_output
            )

            dx = dx_f + dx_b.flip([-1])
            ddt = ddt_f + ddt_b.flip([-1])
            dA = dA_f + dA_b.flip([-1])
            dB = dB_f + dB_b.flip([-1])
            dC = dC_f + dC_b.flip([-1])
            dD = dD_f + dD_b
            dz = dz_f + dz_b.flip([-1])
            ddt_bias = ddt_bias_f + ddt_bias_b.flip([-1])
            dinitial_states = dinitial_states_f + dinitial_states_b.flip([-1])

        else:
            batch = dout.shape[0]
            dy_rms = rearrange(dout, "b s h padding -> (b s) (h padding)")
            dz = rearrange(dz, "b l dilation -> (b l) dilation")
            x_rms = rearrange(out_x_f, "b s h padding -> (b s) (h padding)")
            z_rms = rearrange(z, "b s h padding -> (b s) (h padding)")
            out1_recompute = rearrange(out1_recompute, "b s dilation -> (b s) dilation") if recompute_output else None
            dout, drmsnorm_weight, _, dz, *rest = _layer_norm_bwd(dy_rms, x_rms, rmsnorm_weight, None, ctx.rmsnorm_eps, None, rstd_f, z_rms, group_size=dim//ctx.ngroups, norm_before_gate=ctx.norm_before_gate, is_rms_norm=True, recompute_output=recompute_output, dz=dz, out=out1_recompute if recompute_output else None)
            out_for_linear = out_recompute if recompute_output else None
            dout = rearrange(dout, "(b s) (h padding) -> b s h padding", b=batch, p=headdim)
            dx, ddt, dA, dB, dC, dD, _, ddt_bias, dinitial_states = _mamba_chunk_scan_combined_bwd(
                dout, x, dt, A, B, C, out_x_f, ctx.chunk_size, D=D, z=None, dt_bias=dt_bias, initial_states=initial_states, dfinal_states=dfinal_states, seq_idx=seq_idx, dt_softplus=True, dt_limit=ctx.dt_limit, dx=dx, ddt=ddt_given, dB=dB, dC=dC
            )


            x_rms_b = rearrange(out_x_b, "b s h padding -> (b s) (h padding)")
            dout_b, drmsnorm_weight_b, _, dz_b, *rest_b = _layer_norm_bwd(dy_rms.flip([-1]), x_rms_b, rmsnorm_weight.flip([-1]), None, ctx.rmsnorm_eps, None, rstd_b, z_rms.flip([-1]), group_size=dim // ctx.ngroups, norm_before_gate=ctx.norm_before_gate, is_rms_norm=True, recompute_output=recompute_output, dz=dz.flip([-1]), out=out1_recompute.flip([-1]) if recompute_output else None)
            out_for_linear_b = out_recompute.flip([-1]) if recompute_output else None
            dout_b = rearrange(dout_b, "(b s) (h padding) -> b s h padding", b=batch, p=headdim)
            dx_b, ddt_b, dA_b, dB_b, dC_b, dD_b, _, ddt_bias_b, dinitial_states_b = _mamba_chunk_scan_combined_bwd(
                dout_b, x.flip([-1]), dt.flip([-1]), A_b, B.flip([-1]), C.flip([-1]), out_x_b, ctx.chunk_size, D=D, z=None, dt_bias=dt_bias, initial_states=initial_states,
                dfinal_states=dfinal_states, seq_idx=seq_idx, dt_softplus=True, dt_limit=ctx.dt_limit, dx=dx.flip([-1]),
                ddt=ddt_given, dB=dB.flip([-1]), dC=dC.flip([-1])
            )

            dout = dout + dout_b.flip([-1])
            dx = dx + dx_b.flip([-1])
            ddt = ddt + ddt_b.flip([-1])
            dA = dA + dA_b.flip([-1])
            dB = dB + dB_b.flip([-1])
            dC = dC + dC_b.flip([-1])
            dD = dD + dD_b
            dz = dz + dz_b.flip([-1])
            ddt_bias = ddt_bias + ddt_bias_b.flip([-1])
            dinitial_states = dinitial_states + dinitial_states_b.flip([-1])
            drmsnorm_weight = drmsnorm_weight + drmsnorm_weight_b.flip([-1])
...
def bimamba_split_conv1d_scan_combined(zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, A_b, D, chunk_size, initial_states=None, seq_idx=None, dt_limit=(0.0, float("inf")), return_final_states=False, activation="silu", rmsnorm_weight=None, rmsnorm_eps=1e-6, outproj_weight=None, outproj_bias=None, headdim=None, ngroups=1, norm_before_gate=True):
    """
    Argument:
        zxbcdt: (batch, seqlen, 2 * dim + 2 * ngroups * dstate + nheads) where dim == nheads * headdim
        conv1d_weight: (dim + 2 * ngroups * dstate, width)
    ...
    Return:
        out: (batch, seqlen, dim)
    """
    return BiMambaSplitConv1dScanCombinedFn.apply(zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, A_b, D, chunk_size, initial_states, seq_idx, dt_limit, return_final_states, activation, rmsnorm_weight, rmsnorm_eps, outproj_weight, outproj_bias, headdim, ngroups, norm_before_gate)
...
def mamba_split_conv1d_scan_ref(zxbcdt, conv1d_weight, conv1d_bias, dt_bias, A, D, chunk_size, dt_limit=(0.0, float("inf")), activation="silu", rmsnorm_weight=None, rmsnorm_eps=1e-6, outproj_weight=None, outproj_bias=None, headdim=None, ngroups=1, norm_before_gate=True):
    ...
    out = rearrange(out, "b s h padding -> b s (h padding)")
    if rmsnorm_weight is not None:
        out = rmsnorm_fn(out, rmsnorm_weight, None, z=rearrange(z, "b l h padding -> b l (h padding)"), eps=rmsnorm_eps,
