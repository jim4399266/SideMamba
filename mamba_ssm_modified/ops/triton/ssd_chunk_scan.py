...
def chunk_scan_ref(B, C, x, dt, dA_cumsum, prev_states, D=None, z=None):
    ...
    assert seqlen == nchunks * chunk_size
    assert C.shape == B.shape
    B = repeat(B, "b l g dilation -> b l (g h) dilation", h=nheads // ngroups)
    C = repeat(C, "b l g dilation -> b l (g h) dilation", h=nheads // ngroups)
    ...
    out = torch.einsum('bchls,bhcs,bcshp->bclhp', scores_decay.to(x.dtype), dt.to(x.dtype),
                       rearrange(x, "b (c s) h padding -> b c s h padding", c=nchunks))
    ...
    out = out + out_prev
    out = rearrange(out, "b c l h padding -> b (c l) h padding")
    if D is not None:
        if D.dim() == 1:
            D = rearrange(D, "h -> h 1")
        out = out + x * D
