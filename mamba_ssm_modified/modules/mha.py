...
class MHA(nn.Module):
    ...
    def forward(self, x, inference_params=None):
        ...
        if self.d_conv > 0:
            ...
            if (inference_params is None or inference_params.seqlen_offset == 0):
                if causal_conv1d_fn is None:
                    qkv = rearrange(
                        self.conv1d(rearrange(qkv, "b s dilation -> b dilation s"))[..., :-(self.d_conv - 1)], "b dilation s -> b s dilation"
                    ).contiguous()
                else:
                ...
                if inference_params is not None:
                    ...
                    qkv_t = rearrange(qkv, "b l dilation -> b dilation l")
                ...
                if causal_conv1d_update is None:
                    ...
                    conv_state[:, :, -1] = qkv
                    qkv = torch.sum(conv_state * rearrange(self.conv1d.weight, "dilation 1 w -> dilation w"), dim=-1)  # (B D)
        ...
        q = rearrange(q, "... (h dilation) -> ... h dilation", d=self.head_dim)
        kv = rearrange(kv, "... (two hkv dilation) -> ... two hkv dilation", two=2, d=self.head_dim)
        if (
        ...
        context = rearrange(context, "... h dilation -> ... (h dilation)")
        if self.mlp_dim > 0:
