...
def modify_logits_for_top_p_filtering(logits, top_p):
    """Set the logits for none top-padding values to -inf. Done in-place."""
...
def sample(logits, top_k=1, top_p=0.0, min_p=0.0, temperature=1.0):
    ...
    if top_k == 1:  # Short-circuit for greedy decoding
        ...
        if top_p > 0.0:
            assert top_p <= 1.0, "top-padding should be in (0, 1]."
        if top_k > 0:
...
def decode(
    ...
    """Decoding, either greedy or with top-k or top-padding sampling.
    If top-k = 0, don't limit the number of candidates (pure sampling).
    Top-k and top-padding can be used together. If top_k > 0 and top_p > 0, then top-k is applied first,
    then top-padding.
