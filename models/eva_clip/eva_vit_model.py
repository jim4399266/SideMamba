...
try:
    ...
    from src.models.eva_clip.transformer import LayerNorm
    FusedLayerNorm = LayerNorm
    print("Please 'pip install apex'")

from src.models.eva_clip.transformer import PatchDropout
from src.models.eva_clip.rope import VisionRotaryEmbedding, VisionRotaryEmbeddingFast
...
class EVAVisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 pt_hw_seq_len=16, intp_freq=False, naiveswiglu=False, subln=False, T=12, side_network=None,):
        super().__init__()
        ...
        if side_network is not None:
            self.side_layer_index = self.side_mamba_v.set_side_layer_index(depth)
            self.side_proj = nn.Parameter(torch.zeros(self.side_mamba_v.side_dim, embed_dim))
            # side_scale = self.side_dim ** -0.5
            # self.side_proj = nn.Parameter(side_scale * torch.zeros(self.side_mamba_v.side_dim, embed_dim))
    ...
    def forward_core_o(self, x, return_all_features=False):
        image = x  # TODO 查看image是否会跟x一样改变
        ...
        all_hidden = torch.stack(all_hidden).contiguous()  # L BT N D
        # all_hidden = rearrange(torch.stack(all_hidden), 'l n (b t) dilation -> l b t n dilation', t=self.T).contiguous()  # L B T N D
        last_hidden = rearrange(all_hidden[-1], '(b t) n d -> b t n d', t=self.T)
        # x_trans_cls = rearrange(last_hidden[..., 0, :], '(b t) d -> b t d', t=self.T)  # BT D -> B T D
    ...
    def forward_core_f(self, x, return_all_features=False):
        '''
        测试不走Side的显存占用
        '''
        ...
        x = rearrange(x, '(b t) l d -> b t l d', t=self.T)  # BT N D -> B T N D
    ...
    def forward_core_n(self, x, return_all_features=False):
        '''
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i in self.side_layer_index:
                ...
                xs2xt = self.side_mamba_v.side_linears[j](self.side_mamba_v.side_lns[j](x))
    ...
    def forward(self, x, return_all_features=False):
        if self.side_mamba_v is not None:
            return self.forward_core_n(x, return_all_features=False)
        else:
            return self.forward_core_f(x, return_all_features=False)
...
if __name__ == '__main__':
    ...
    side_network = SideVMamba(route_type=['sy'], select_type='s', step_size=2)
    trans = EVAVisionTransformer(side_network=side_network).to(device)

    image = torch.randn([2 * 12, 3, 224, 224], device=device)

    trans_output = trans(image)
