...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None, use_checkpoint=True):
        if use_checkpoint:
            attn_out = checkpoint(self.attention, self.ln_1(x), attn_mask)
...
class SideResidualAttentionBlock(nn.Module):
    def __init__(
            self,
            d_model: int,
            n_head: int,
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None, use_checkpoint=True, position_embedding=None):
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.side_linears = []
        self.side_lns = []
        self.side_layers_mode = 'interval' # all, interval, top
        ...
        for i in range(layers):
            self.resblocks.append(ResidualAttentionBlock(width, heads, mlp_ratio, ls_init_value=ls_init_value, act_layer=act_layer, norm_layer=norm_layer, xattn=xattn))
            if i in self.side_layers:
                self.side_transformer.append(SideResidualAttentionBlock(self.side_dim, self.side_dim//64, ls_init_value=ls_init_value, act_layer=act_layer, norm_layer=norm_layer, xattn=xattn))
        ...
        self.side_spatial_position_embeddings = nn.Parameter(torch.randn(32, self.side_dim) / self.side_dim ** 0.5)
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None):
        ...
        for i in range(self.layers):
            ...
            if k == 0:
                ...
                x_side = x_side + xs2xt
            x_side = checkpoint(self.side_transformer[k], x_side, attn_mask, True, self.side_spatial_position_embeddings)
            # x_side = self.side_transformer[k](x_side, self.side_spatial_position_embeddings)
            k += 1
        return x, x_side
