...
class RawFramesExtractorCV2():
    ...
    def _sample_indices(self, video_list):
        if not self.dense_sample:
            ...
            if average_duration > 0:
                # offsets += list(np.multiply(list(range(num_segments)), average_duration) + randint(average_duration,size=num_segments))
    ...
    def video_to_tensor(self, frames_folder, preprocess,  start_time=None, end_time=None):
        ...
        if self.strategy == 1:  # for val & test
            frame_indexs = np.arange(0, frameCount, frameCount // self.num_segments) + 1
            ...
            if start_sec and end_sec:   # no through
                frame_indexs = [x for x in frame_indexs if x>=start_sec*30 and x<=end_sec*30]
            if frame_indexs[-1] > frameCount:
                frame_indexs = frame_indexs[:-1]                
        elif self.strategy == 4:
...
class RawFramesExtractorCV2_Image():
    def __init__(self, centercrop=False, size=224, num_segments=-1, dense_sample=False, random_shift=False, strategy=1, return_images=False):
        self.centercrop = centercrop
        self.size = size
        self.num_segments = num_segments
        self.dense_sample = dense_sample
    ...
    def video_to_tensor(self, frames_folder, preprocess, start_time=None, end_time=None):
        ...
        for ind in frame_indexs:
            p = int(ind)
            raw_imgs = Image.open(os.path.join(frames_folder, self.image_tmpl.format(p))).convert("RGB")
            segs_imgs = [preprocess(raw_imgs)]
            if self.return_images:
                raw_images.append(raw_imgs.resize((224, 224)))
            images.extend(segs_imgs)
            if p < len(frames_list):
                p += 1
