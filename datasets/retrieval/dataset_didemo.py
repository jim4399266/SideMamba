...
# from src.datasets.rawframes_util import RawFramesExtractor
# from src.datasets.rawvideo_util import RawVideoExtractor
from ..rawframes_util import RawFramesExtractorCV2
...
class DiDeMo_Dataset(Dataset):
    def __init__(
        ...
        elif self.features_path.split('/')[-1] == 'DiDeMo_Frames':
        ...
        self.sentence_num = self.video_num = len(self.iter2video_pairs_dict)
        ...
        try:
            for i in range(len(s)):
                ...
                if len(raw_frame_data.shape) > 3:
                    ...
                    try:
                        assert video_slice.shape == (self.max_frames, 1, 3, self.rawFramesExtractor.size,
                                                     self.rawFramesExtractor.size)
                    except:
                        logger.inhi(f'wrong frame in {idx}')
    ...
    def __getitem__(self, feature_idx):
        ...
        video_index = feature_idx
        # feature_idx : 文本的编号      video_index: 视频的编号
        return pairs_text, pairs_mask, pairs_segment, video, video_mask, feature_idx, video_index